<?php
//this version has no hard coded domain or secret- hopefully I can use this for folks on other canvas instances
...
//note to self. these session variables should not be set= $_SESSION['isInstructor'] = $_SESSION['contextid']= $_SERVER['uid']	
	...
	die("Access denied. If you have just approved this integration, refresh your browser now. This message should not appear a second time unless you delete a U Delaware auth token.");
...
if(!empty($data)){
	...
	if($context->isInstructor()){
		...
		echo ' <a href="canvas_roster_shared.php" >Download Template</a>';
	}
	echo "</blockquote>";
...
<h4>Upload CSV</h4>
<blockquote>Your file should have student names in the first column, and ids in the second (SISID or email address). All other columns will be treated as grades. Any row containing an SISID or email address in the second column will be visible only to that student. Rows with an empty field in the first column (no name) will be shown to all. <a href="canvas_roster_shared.php" >Download Template</a></blockquote>
