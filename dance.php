...
if($_REQUEST['dance']=='temp'){//form in this page has been submitted
	...
	$state=rand(100,999);//security stuff
		  setcookie('state',$state,0,'/');
				header("Location: https://" .$domain."/login/oauth2/auth?client_id=10000000000369&redirect_uri=https://apps.ats.udel.edu/git/lti/get_temp_token.php&response_type=code&purpose=UD Single_Use_Token&state=" . $state);
	...
	if($_REQUEST['dance']=='domain'){
	 ...
	 $state=rand(100,999);
		  setcookie('state',$state,0,'/');
				header("Location: https://" .$domain."/login/oauth2/auth?client_id=10000000000369&redirect_uri=https://apps.ats.udel.edu/git/lti/get_token_domain.php&response_type=code&purpose=UD ".$_REQUEST['dance']." token&state=" . $state);
}
?>
<html>
<head>
...
<form id="dance" action="/git/lti/dance.php" method="get">
